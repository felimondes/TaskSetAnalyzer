...
def response_time_analysis_rta(df: pd.DataFrame) -> tuple[bool, pd.DataFrame]:
    ...
    # Priority order: use Priority column if present, else DM (by deadline)
    if "Priority" in df.columns:
        work = df.sort_values(by=["Priority", "D_i", "T_i"]).reset_index(drop=True)
    else:
...
def analyze_tasksets_in_folder(folder_path: str) -> list[tuple[str, bool, pd.DataFrame]]:
    """
    Analyze all tasksets in a folder.
    
    Args:
        folder_path: Relative path to the folder containing CSV files
        
    Returns:
        List of tuples: [(csv_id, schedulable, results_df), ...]
    """
    parser = Parser()
    dfs = parser.taskSetParser(folder_path)
    
    results = []
    for df in dfs:
        csv_id = df["csv_id"].iloc[0] if "csv_id" in df.columns else "unknown"
        schedulable, result_df = response_time_analysis_rta(df)
        results.append((csv_id, schedulable, result_df))
